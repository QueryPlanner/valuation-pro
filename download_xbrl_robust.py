...
class CookieManager:
    ...
    def get_cookies(self) -> Dict[str, str]:
        ...
        print("Or press Enter to use example cookies (may not work):")
        ...
        if sys.stdin.isatty():
            ...
            # Non-interactive mode - use env or default
            cookie_input = os.environ.get('NSE_COOKIES', '')
            if not cookie_input:
                print("  (Using example cookies - non-interactive mode)")
        
        if cookie_input:
            cookies = self._parse_cookie_string(cookie_input)
        else:
            # Use default/example cookies
            print("\n⚠ Using example cookies (likely expired)")
            print("For reliable access, please paste your own cookies.")
            cookies = {
                'nsit': 'JRJcpVyWnOXyfgP3UIZ9UmxR',
                'AKA_A2': 'A'
            }
...
class NSEXBRLDownloader:
    ...
    def download_reports(self, symbol: str, output_dir: str = 'xbrl_downloads',
                        quarterly: int = 0, annual: int = 0,
                        consolidated_only: Optional[bool] = None) -> List[dict]:
        ...
        if quarterly > 0:
            ...
            if results:
                ...
                downloaded.extend(self._download_reports_list(
                    results, output_dir, symbol, 'Quarterly', quarterly, consolidated_only
                ))
        ...
        if annual > 0:
            ...
            if results:
                ...
                downloaded.extend(self._download_reports_list(
                    results, output_dir, symbol, 'Annual', annual, consolidated_only
                ))
    ...
    def _download_reports_list(self, results: List[dict], output_dir: str,
                               symbol: str, period_type: str, limit: int,
                               consolidated_only: Optional[bool]) -> List[dict]:
...
"""
    ...
    parser.add_argument('--quarters', type=int, default=4, help='Number of quarterly reports (default: 4)')
    ...
    quarterly = 0 if args.annual > 0 and args.quarters == 4 else args.quarters
    annual = args.annual
