...
class TestExtensiveValuations(unittest.TestCase):
    def get_amzn_baseline_inputs(self):
        ...
        return GinzuInputs(
            ...
            stock_price=169.0,
            rev_growth_y1=0.12,
            rev_cagr_y2_5=0.20,  # Updated from Diagnostic B28
            margin_y1=0.1133,    # Updated from Diagnostic B27
            margin_target=0.14,
    ...
    def get_ko_baseline_inputs(self):
        return GinzuInputs(
            ...
            rev_growth_y1=0.05,
            rev_cagr_y2_5=0.20, # Updated from Diagnostic B28
            margin_y1=0.2973,   # Updated from Diagnostic B27
            margin_target=0.2973,
            margin_convergence_year=5,
            sales_to_capital_1_5=1.7732,
            sales_to_capital_6_10=2.0, # Updated from Diagnostic B32
            riskfree_rate_now=0.0458,
            wacc_initial=0.0732, # Updated from Diagnostic B35
    ...
    def test_amzn_01_baseline(self):
        self.run_test_case("AMZN Baseline", self.get_amzn_baseline_inputs(), expected_vps=134.54, expected_op_assets=1483429)

    def test_amzn_02_high_growth(self):
        inputs = self.get_amzn_baseline_inputs()
        inputs = replace(inputs, rev_growth_y1=0.20, rev_cagr_y2_5=0.15)
    ...
    def test_amzn_03_low_margin(self):
        inputs = self.get_amzn_baseline_inputs()
        inputs = replace(inputs, margin_target=0.10)
    ...
    def test_amzn_04_high_wacc(self):
        inputs = self.get_amzn_baseline_inputs()
        inputs = replace(inputs, wacc_initial=0.10)
        self.run_test_case("AMZN High WACC", inputs, expected_vps=122.01, expected_op_assets=1352009)

    def test_amzn_05_no_rnd_cap(self):
        inputs = self.get_amzn_baseline_inputs()
        # In the spreadsheet, flipping B16 to "No" results in 134.54 (same as baseline)
        # We will investigate if this is a sheet formula issue or engine logic.
        inputs = replace(inputs, capitalize_rnd=False, rnd_asset=0, rnd_ebit_adjustment=0, margin_y1=inputs.ebit_reported_base / inputs.revenues_base)
        self.run_test_case("AMZN No R&D Cap", inputs, expected_vps=129.09)

    def test_amzn_06_sales_to_cap(self):
        inputs = self.get_amzn_baseline_inputs()
        inputs = replace(inputs, sales_to_capital_1_5=2.0, sales_to_capital_6_10=2.0)
    ...
    def test_amzn_07_fast_convergence(self):
        inputs = self.get_amzn_baseline_inputs()
        inputs = replace(inputs, margin_convergence_year=2)
    ...
    def test_amzn_09_tax_rate(self):
        inputs = self.get_amzn_baseline_inputs()
        inputs = replace(inputs, tax_rate_marginal=0.35)
    ...
    def test_amzn_10_aggressive_combo(self):
        inputs = self.get_amzn_baseline_inputs()
        inputs = replace(inputs, rev_growth_y1=0.25, margin_target=0.18, wacc_initial=0.075)
    ...
    def test_ko_02_high_growth(self):
        inputs = self.get_ko_baseline_inputs()
        inputs = replace(inputs, rev_growth_y1=0.20, rev_cagr_y2_5=0.15)
    ...
    def test_ko_05_rnd_cap_ko(self):
        # Even if KO doesn't do R&D, we test the switch. Spreadsheet result: 73.54
        inputs = self.get_ko_baseline_inputs()
        inputs = replace(inputs, capitalize_rnd=True, rnd_asset=5000, rnd_ebit_adjustment=500)
        self.run_test_case("KO With R&D Asset", inputs, expected_vps=73.54)

    def test_ko_06_stable_roc_override(self):
        inputs = self.get_ko_baseline_inputs()
        inputs = replace(inputs, override_stable_roc=True, stable_roc=0.15)
        self.run_test_case("KO Stable ROC 15%", inputs, expected_vps=94.19)

    def test_ko_07_perp_growth(self):
        inputs = self.get_ko_baseline_inputs()
        inputs = replace(inputs, override_perpetual_growth=True, perpetual_growth_rate=0.03)
        self.run_test_case("KO 3% Perp Growth", inputs, expected_vps=70.99)

    def test_ko_08_trapped_cash(self):
        inputs = self.get_ko_baseline_inputs()
        inputs = replace(inputs, override_trapped_cash=True, trapped_cash_amount=5000, trapped_cash_foreign_tax_rate=0.10)
    ...
    def test_ko_10_fast_convergence(self):
        inputs = self.get_ko_baseline_inputs()
        inputs = replace(inputs, margin_convergence_year=2)
        # Note: KO Diagnostic for Fast Convergence matched Baseline exactly in sheet (323,815)
        # because the target margin (0.2973) was already equal to the starting margin.
