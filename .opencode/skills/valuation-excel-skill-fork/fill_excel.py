...
def fill_valuation_excel(
    company_name, inputs_json_path, template_path, output_path, price=None, rf_rate=None, erp=0.055, ticker=None
):
    ...
    write_cell(
        "B19", fin_data.get("Cross_holdings_and_other_non_operating_assets", {}).get("Most_Recent_12_months", 0.0)
    )
    ...
    if "Stories to Numbers" in wb.sheetnames:
        ...
        ws_story["A3"] = (
            "GUIDANCE: Read the 'Management Discussion & Analysis' (MD&A) section in the Annual Report. What is the company's core business model? Who are their competitors? Are they a disruptor or an incumbent? Write a narrative here tying their business strategy to their ability to generate cash flows, drive growth, and manage risk."
        )
        ws_story["G9"] = (
            "GUIDANCE: Link to Growth Story. Look at historical revenue trends and management's future guidance. Why will they grow at this rate?"
        )
        ws_story["G10"] = (
            "GUIDANCE: Link to Profitability. Are they cutting costs or scaling? Compare current margins to industry averages to set a realistic target."
        )
        ws_story["G11"] = (
            "GUIDANCE: Check the effective tax rate in the income statement vs the marginal corporate tax rate of their home country."
        )
        ws_story["G12"] = (
            "GUIDANCE: Link to Capital Efficiency. How much reinvestment (CapEx, R&D, Acquisitions) is needed to drive the growth story above?"
        )
        ws_story["G13"] = (
            "GUIDANCE: Link to Competitive Advantage (Moat). A high ROC requires strong barriers to entry. Do they have them?"
        )
        ws_story["G14"] = (
            "GUIDANCE: Link to Risk Profile. Look at their debt load, geographical exposure, and business cyclicality to justify the Cost of Capital."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Yahoo Finance ticker symbol to automatically fetch price and risk-free rate",
    )
    args = parser.parse_args()

    fill_valuation_excel(
        args.company, args.inputs, args.template, args.output, args.price, args.rf_rate, args.erp, args.ticker
    )
